...
def test_packet_detail_returns_200(tmp_path: Path) -> None:
...
def test_packet_detail_shows_fields(tmp_path: Path) -> None:
    client = _setup_app(tmp_path, [_PACKET])
    resp = client.get("/packet/eeee-0005")
...
def test_packet_detail_inline_screenshot(tmp_path: Path) -> None:
    png = tmp_path / "eeee-0005.png"
    png.write_bytes(b"\x89PNG")
    packet = {**_PACKET, "artifact_refs": [str(png)]}
    client = _setup_app(tmp_path, [packet])
...
def test_packet_detail_no_screenshot_when_file_missing(tmp_path: Path) -> None:
...
def test_packet_detail_404_for_unknown_id(tmp_path: Path) -> None:
